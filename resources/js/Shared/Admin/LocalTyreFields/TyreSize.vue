<template>
    <div
        class="relative flex items-center justify-center border-gray-300 h-full text-xs group"
    >
        <span v-if="!editMode" v-text="text"></span>
        <div v-if="editable" class="space-x-0.5 flex items-center text-[11px]" :class="{'hidden': !editMode}">
            <select ref="widthEl" :value="width" class="bg-transparent border border-black rounded-sm w-8 outline-0 appearance-none text-center">
                <option v-for="width in widths" :value="width" v-text="width"></option>
            </select>
            <select ref="profileEl" :value="profile" class="bg-transparent border border-black rounded-sm w-6 outline-0 appearance-none text-center">
                <option v-for="profile in profiles" :value="profile" v-text="profile"></option>
            </select>
            <select ref="rimEl" :value="rim" class="bg-transparent border border-black rounded-sm w-6 outline-0 appearance-none text-center">
            ...
            <div class="relative w-4 h-4">
                ...
                <input class="w-4 h-4" type="checkbox" :checked="commercial" ref="commercialInp">
            </div>
        </div>
...
const rimEl = ref(null)
const commercialInp = ref(null)

function saveEdit () {
    editMode.value = false;

    let size = { width: widthEl.value.value, profile: profileEl.value.value, rim: rimEl.value.value, commercial: commercialInp.value.checked }
