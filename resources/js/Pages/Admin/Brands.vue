<template>
    ...
    <div class="container">
        <div class="lg:w-1/3 w-full mx-auto pt-8">
            <p class="font-semibold text-3xl text-center mb-8">Manage Brands</p>
            ...
            <div class="flex flex-col space-y-2">
                <div v-for="brand in brands" class="flex flex-row items-center justify-between bg-gray-100 py-2 px-2 rounded-lg">
                    <span class="text-sm font-semibold" v-text="brand.brand"></span>

                    <div class="flex flex-row space-x-2">
                        <button @click="deleteBrand(brand.id)" class="block rounded bg-red-400 text-white sm:text-sm text-xs font-semibold py-1 px-3">
...
function submit() {
    form.post('/admin/manage-tyre-brands', {
        preserveScroll: true,
        preserveState: true,
        onSuccess: () => {
            form.reset();
...
function deleteBrand(id) {
    if(confirm('Are you sure you want to delete this?')) {
        router.delete('/admin/manage-tyre-brands/' + id, {
            preserveScroll: true,
            preserveState: true,
        });
    }
