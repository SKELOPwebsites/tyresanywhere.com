    ...
    <Container>
        <div class="lg:w-1/3 w-full mx-auto pt-8">
            ...
            <div class="flex flex-col space-y-2">
                <div v-for="area in areas" class="flex flex-row items-center justify-between bg-gray-100 py-2 px-2 rounded-lg">
                    <span class="text-sm font-semibold" v-text="area.area"></span>

                    <div class="flex flex-row space-x-2">
                        <button @click="deletePostcode(area.id)" class="block rounded bg-red-400 text-white sm:text-sm text-xs font-semibold py-1 px-3">
...
function deletePostcode(id) {
    router.delete('/admin/covered-areas/' + id, {
        preserveScroll: true,
        preserveState: true,
    });
}
